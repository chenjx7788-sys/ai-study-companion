...
const steps = [
  { name: '导入材料', desc: '在材料库上传或直接引用本地文件（文件 / 文件夹均可），PDF/PPT/Word/Markdown/EPUB/图片/音视频自动解析成可读文本' },
  { name: '理解 & 沉淀', desc: '学习页做 AI 总结、三色划线、改写扩写续写总结；AI 问答的回答可一键转成笔记，存进知识库' },
  { name: '复盘 & 抗遗忘', desc: '用复习巩固翻卡对抗遗忘，在数据统计查看学习看板与 AI 周报；也能让 AI 把材料讲成一段播客，碎片时间接着听' }
]

const modules = [
  { path: '/', name: '材料库', desc: '上传并管理所有学习材料', points: ['文件夹管理：多级嵌套、批量导入保留目录层级、夹内搜索', '本地文件直引：引用原文件或复制副本，不重复占磁盘', '内置 Markdown 文档编辑器，直接在库内撰写', 'EPUB 电子书：文本 / 原文双视图，保留原书排版与插图', '图片与扫描版 PDF 本地 OCR 识别', '标签分类（可预置标签）、标题/全文搜索、阅读进度'] },
  { path: '/study', name: '学习页', desc: '三栏沉浸式阅读器', points: ['目录 / 阅读器 / AI 面板三栏可拖拽调宽', '文本视图 + 原文视图（PDF / Word / EPUB）+ 音视频转写，PDF 可全屏', '黄/绿/蓝三色划线，跨视图持久化', '选中即用：AI 解读、改写扩写总结、转笔记、复制', 'md 文档富文本编辑，修订同步更新知识库检索'] },
  { path: '/study/ai', name: 'AI 理解', desc: '读懂每一份资料', points: ['脉络摘要流式生成，(P页码) 出处可点击定位', '核心知识点卡片，自动提取并回链原文', '划线 AI 解读 + 持续追问链', '「测一测」AI 一键出题，随学随测', 'AI 加工四件套：改写 / 扩写 / 续写 / 总结', '摘要 / 知识点 / 追问均可一键转笔记'] },
  { path: '/knowledge', name: '知识库', desc: '你的第二大脑，AI 答案来源', points: ['原文与笔记自动向量化，修改即重索引', '多路召回：向量语义 + 全文精确双路检索', '笔记权重可调（默认更高，更易命中）', '按材料 / 按笔记双视图，笔记按问答 / AI / 周报 / 播客 / 手动分类可筛'] },
  ...
  { path: '/podcast', name: 'AI 播客', desc: '把知识讲给你听', points: ['来源：单篇材料 / 划线文档 / 笔记 / 错题卡', '两步生成：先出知识简报，再出对话脚本（含单人精讲）', '14 种音色可逐个试听，可选内置氛围音或自传背景音乐', '本地合成、按句跟读，可导出文稿与 SRT 字幕'] },
